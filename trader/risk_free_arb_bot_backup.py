...
class RiskFreeArbBot(BaseArbBot):
    ...
    def run(self):
        ...
        # if backtesting
            ...
            if self.MANUAL_TRADE_ON:
                logging.critical("mm1_buy_manual: %s, mm1_sell_manual: %s, mm2_buy_manual: %s, mm2_sell_manual: %s" %
                                 (self.mm1_buy_manual_flag, self.mm1_sell_manual_flag,
                                  self.mm2_buy_manual_flag, self.mm2_sell_manual_flag))
                logging.critical("manual new: %d, manual rev: %d" % (self.new_manual_count, self.rev_manual_count))
                logging.critical("last manual order: %s" % self.last_manual_order)
