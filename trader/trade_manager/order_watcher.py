...
class OrderWatcher(Thread):
    ...
    def __init__(self, order: Order, interval_sec: int, risk_flag_sec: int):
        super().__init__()
        self.order = order
        self.interval_sec = interval_sec
        self.risk_flag_sec = risk_flag_sec
    ...
    def run(self):
        ...
        while self.order.status is not OrderStatus.FILLED:
            ...
            if not self.is_at_risk:
                ...
                if time_check > self.risk_flag_sec:
                    self.is_at_risk = True
                    message = "Order %s has exceeded risk flag time! " \
